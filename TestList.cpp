...
class Stack {
public:
    bool isEmpty() const { return theList.empty(); }

    const Object &top() const { return theList.front(); }

    void push(const Object &x) { theList.push_front(x); }
...
public:
    ...
    const Object &getFront() const { return theList.front(); }

    void enqueue(const Object &x) { theList.push_back(x); }
...
class CompareList {
public:
    bool operator()(const List<int> &lhs, const List<int> &rhs) const { return lhs.size() < rhs.size(); }
};
